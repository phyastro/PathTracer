...
vec3 Rendering(in vec3 inColor) {
	uvec2 xy = uvec2(gl_FragCoord.x, resolution.y - gl_FragCoord.y);
...
void main() {
	vec3 prevRendererColor = texelFetch(prevRendererImage, ivec2(gl_FragCoord.xy), 0).xyz;
	rendererColor = vec4(Rendering(prevRendererColor), 1.0);
	processorColor = vec4(Processing(rendererColor.xyz), 1.0);
}